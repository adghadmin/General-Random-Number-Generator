...
always @(posedge I_sys_clk) begin
	if(R_rst_2d) begin
		...
		R_seqgen_en_pos <= {R_seqgen_en_pos[10:0],W_seqgen_en_pos};
	end
end
...
reg												R_start_flag= 'd0	;
reg					[2:0]						R_clk_cnt 	= 'd0	;
always @(posedge I_sys_clk) begin
	if(R_rst_2d) begin
		R_start_flag <= 'd0;
	end
	else if(R_seqgen_en_pos[11]) begin
		R_start_flag <= 'd1;
	end
	else if(W_seqgen_en_neg) begin
		R_start_flag <= 'd0;
	end
	else begin
...
always @(posedge I_sys_clk) begin
	...
	else if(R_clk_cnt == 'd3) begin //x_n = (a_1 x_{n-1} + a_5 x_{n-5}) mod m
...
reg											R_seq_dvld	= 'd0;
reg					[Q-1:0]					R_seq_data	= 'd0;
always @(posedge I_sys_clk) begin
	if(R_rst_2d) begin
		R_seq_data <= 'd0;
		R_seq_dvld <= 'd0;
	end
	else if(R_seqgen_en_pos[7]) begin
		R_seq_data <= R_value_x[0][31:0];
		R_seq_dvld <= 'd1;
	end
	else if(R_seqgen_en_pos[8]) begin
		R_seq_data <= R_value_x[1][31:0];
		R_seq_dvld <= 'd1;
	end
	else if(R_seqgen_en_pos[9]) begin
		R_seq_data <= R_value_x[2][31:0];
		R_seq_dvld <= 'd1;
	end
	else if(R_seqgen_en_pos[10]) begin
		R_seq_data <= R_value_x[3][31:0];
		R_seq_dvld <= 'd1;
	end
	else if(R_seqgen_en_pos[11]) begin
		R_seq_data <= R_value_x[4][31:0];
		R_seq_dvld <= 'd1;
	end
	else if(R_clk_cnt == 'd3) begin
		R_seq_data <= remain;
		...
		R_seq_data <= R_seq_data;
		R_seq_dvld <= 'd0;
	end
end

assign O_seq_dvld = R_seq_dvld; //sequence data valid
assign O_seq_data = R_seq_data[31:32-Q]  ;	//sequence data
endmodule